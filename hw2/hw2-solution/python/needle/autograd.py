...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        # Sum up all gradients for this node
        node.grad = Tensor(sum(node_to_output_grads_list[node]),dtype =out_grad.dtype)   # vi^{-}
        # Compute gradient contributions for each input node
        if node.op is not None:
            # vi^{-} * \partial to input[0],[1]
            in_grads = node.op.gradient_as_tuple(node.grad, node)
            # Accumulate gradients in the map
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    """
    ### BEGIN YOUR SOLUTION
    visited = {id(node): False for node in node_list}
    topo_order = []
